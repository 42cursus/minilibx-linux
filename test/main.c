...
int main()
{
	...
	if (!(win1 = mlx_new_window(mlx, WIN1_SX, WIN1_SY, (char *) "Title1")))
	{
		printf(" !! KO !!\n");
		exit(1);
	}
	...
	printf(" => Fill Image1 ...");
	color_map_2((void *) data1, bpp1, sl1, IM1_SX, IM1_SY, endian1, 1);
	printf("OK (pixmap : %d)\n", (int) ((t_img *) im1)->pix);
	...
	printf(" => Fill Image3 ...");
	color_map_2((void *) data3, bpp3, sl3, IM3_SX, IM3_SY, endian3, 1);
	printf("OK (pixmap : %d)\n", (int) ((t_img *) im3)->pix);
	...
	printf(" => String ...");
	mlx_string_put(mlx, win1, 5, WIN1_SY / 2, 0xFF99FF,
				   (char *) "String output");
	mlx_string_put(mlx, win1, 15, WIN1_SY / 2 + 20, 0x00FFFF,
				   (char *) "MinilibX test");
	printf("OK\n");
	sleep(2);

	printf(" => Xpm from file ...");
	if (!(im2 = mlx_xpm_file_to_image(mlx, (char *) "open.xpm", &xpm1_x,
									  &xpm1_y)))
	{
		printf(" !! KO !!\n");
		exit(1);
	...
	printf(" => 2nd window,");
	win2 = mlx_new_window(mlx, WIN1_SX, WIN1_SY, (char *) "Title2");
	...
	color_map_2((void *) data4, bpp4, sl4, IM3_SX, IM3_SY, endian4, 2);

	printf(" 3rd window, Installing hooks ...");
	win3 = mlx_new_window(mlx, WIN1_SX, WIN1_SY, (char *) "Title3");
	mlx_expose_hook(win1, expose_win1, 0);
	mlx_mouse_hook(win1, (void *) mouse_win1, 0);
	mlx_key_hook(win1, (void *) key_win1, 0);
	mlx_expose_hook(win2, expose_win2, 0);
	mlx_mouse_hook(win2, (void *) mouse_win2, 0);
	mlx_key_hook(win2, (void *) key_win2, 0);
	mlx_key_hook(win3, (void *) key_win3, 0);
	...
	while (y--)
	{
		...
		while (x--)
		{
			...
			while (dec--)
				...
				{
					if (endian)
						*(ptr + x * opp + dec) = ((unsigned char *) (&color2))[
							4 - opp + dec];
					else
						*(ptr + x * opp +
